...
#' @param nbatch Number of batches to run (number of samples to return)
#' @param blen Length of each batch
#' @param depth The depth of the likelihood approximation. Higher values result in better approximations. The default value is 3.
#' 
...
inference <- function(demography, ili, mon_pop, n_pos, n_samples, 
        ...
        risk_group_map, risk_ratios, lprior, lpeak_prior, nburn = 0, nbatch = 1000, blen = 1, depth = 3)
{
  uk_defaults <- F
  ...
  if ("batch" %in% names(initial) || length(dim(initial == 2))) {
    ...
    results <- .inference_cpp_with_covariance(demography, sort(unique(age_group_limits(as.character(age_group_map$from)))),
                                              ...
                                              no_age_groups, no_risk_groups, uk_defaults, nburn, nbatch, blen, depth)
  } else {
