...
To perform parameter inference we need to define a function that returns the (log) likelihood for given parameter values, as dependent on the data and a function which returns the log prior probability of the parameters. These functions will then be passed to `adaptive.mcmc` which will return a posterior sample for the parameter values. We are combining a number of data sources, making the likelihood function relatively complex. The likelihood function needs to perform the following steps (for full details see Baguelin et al., 2013):
...
- Our ILI data and confirmation data is subdivided into 5 age groups, so we need to convert the model results from 7 age groups to the 5 age
- Finally calculate the likelihood of the converted model results given the ILI and confirmation data
...
build.llikelihood <- function()
{
  ...
  llikelihood.f <- function( pars )
  {
    ...
    age.groups <- separate.into.age.groups( age.sizes[,1], 
                                           age.group.limits )
