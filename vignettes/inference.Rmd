...
# calculate it for the (equal tailed) credibility interval of 0% (median) and 98%
cim <- credible.interval.model(ode.results, inference.results$batch)
cim$row.ID <- sapply(cim$row.ID,function(s) as.Date(as.character(s))) 

ggplot( data=cim ) + facet_wrap( ~ column.ID, ncol=3, scales="free" ) +
  geom_line(aes(x=row.ID,y=value,group=aggregate,colour=aggregate,factor=column.ID)) +
