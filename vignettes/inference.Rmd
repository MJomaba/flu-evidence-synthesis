...
**[Need a short description of the underlying model, the parameters that are being estimated here, and whether the package is built to accommodate changes to this model.]** Further details of the underlying model and techniques can be found in Baguelin et al. (2013). 

The methods are computationally intensive and fitting the model can easily take a couple of hours on a powerfull machine. **[It would be useful to see a real example here - the inference ran XX hours and YY minutes on XYZ machine with XX specs.]**

# Examples

## Simple example

The example below shows an example for the case where the data used corresponds to the structure of the data gathered in the UK. **[This structure assumes XX age classes of the following ranges: XX, XX,... XX. And other highlights of this data structure]** 
...
**[Need a little bit more info on the output list `inference.results`, its structure, what information it entails, etc. e.g. Returns a list with the accepted parameters and the corresponding log-likelihood values and a matrix (contact.ids) containing the row numbers of the contacts data used to build the contact matrix. ]**  
