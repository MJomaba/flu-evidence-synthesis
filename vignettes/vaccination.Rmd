...
new_calendar <- change_coverage(calendar, rep(0.8, ncol(calendar)))

# The rest of the simulation is similar as above, but now using the new_calendar
new_vaccine_calendar <- as.vaccination.calendar(efficacy = efficacy, dates = dates, 
                                            coverage = new_calendar, no_age_groups = 7,
                                            no_risk_groups = 3)

new_cases <- sapply(seq(1,nrow(inference.results$batch)), function(i)
{
  sum(vaccinationScenario( age_sizes=age_sizes[,1],
...
ggplot(data=data.frame("cases"=new_cases)) + geom_histogram(aes(x=cases),bins=25)
```
